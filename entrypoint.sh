...
if [ -n "${PEERS:-}" ]; then { printf '{"log":{"loglevel":"warning"},"inbounds":['; inbounds; printf '],"outbounds":[{"tag":"block","protocol":"blackhole"}'; outbounds; printf '],"routing":{"rules":['; rules; printf ']}}'; } >"$D/client.json"; exec xray run -config "$D/client.json"; fi
